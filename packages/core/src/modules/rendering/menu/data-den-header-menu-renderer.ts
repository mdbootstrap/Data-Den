...
export class DataDenHeaderMenuRenderer {
  ...
  constructor(cssPrefix: string, colDef: DataDenColDef, colIndex: number, private PubSub: DataDenPubSub) {
    ...
    const template = `<button class="${this.#cssPrefix}header-menu-toggler">
      <svg xmlns="http://www.w3.org/2000/svg" width="16px" height="100%" viewBox="0 0 16 16" fill="currenColor" class="${this.#cssPrefix
    ...
    const dropdownTemplate = `<div class="${this.#cssPrefix}header-menu-dropdown">
      ...
      ${this.colDef.grouped
        ? `<a href="#" class="${this.#cssPrefix}header-menu-dropdown-item" ref="ungroupColTrigger">Ungroup</a>`
        : `<a href="#" class="${this.#cssPrefix}header-menu-dropdown-item" ref="groupColTrigger">Group</a>`
      }
    </div>`;
  ...
  #initDropdown() {
    ...
    const ungroupColTrigger = this.dropdown.querySelector('[ref="ungroupColTrigger"]');
    const groupColTrigger = this.dropdown.querySelector('[ref="groupColTrigger"]');

    ungroupColTrigger?.addEventListener('click', (event) => {
      event.preventDefault();
      this.PubSub.publish('command:ungroup-column:start', {
        colIndex: this.colIndex,
        group: this.colDef.field,
        context: new Context('command:ungroup-column:start'),
      });
    });

    groupColTrigger?.addEventListener('click', (event: MouseEvent) => {
      event.preventDefault();

      this.PubSub.publish('command:group-column:start', {
        colIndex: this.colIndex,
        group: this.colDef.field,
        context: new Context('command:group-column:start'),
        pageX: event.pageX,
      });
    });
