var Js = (h, t, e) => {
  if (!t.has(h)) throw TypeError('Cannot ' + e);
};
var s = (h, t, e) => (Js(h, t, 'read from private field'), e ? e.call(h) : t.get(h)),
  r = (h, t, e) => {
    if (t.has(h)) throw TypeError('Cannot add the same private member more than once');
    t instanceof WeakSet ? t.add(h) : t.set(h, e);
  },
  a = (h, t, e, i) => (Js(h, t, 'write to private field'), i ? i.call(h, e) : t.set(h, e), e);
var Ys = (h, t, e, i) => ({
    set _(n) {
      a(h, t, n, e);
    },
    get _() {
      return s(h, t, i);
    },
  }),
  o = (h, t, e) => (Js(h, t, 'access private method'), e);
function v(h, t = 'div') {
  const e = document.createElement(t);
  return (e.innerHTML = h), e.firstElementChild;
}
function gi(h, t) {
  let e;
  return (...i) => {
    const n = () => {
      clearTimeout(e), h(...i);
    };
    clearTimeout(e), (e = setTimeout(n, t));
  };
}
const Gt = (h) => {
  if (typeof h != 'object' || h === null) return h;
  if (h instanceof Date) return new Date(h.getTime());
  if (h instanceof Array) return h.reduce((t, e, i) => ((t[i] = Gt(e)), t), []);
  if (h instanceof Object) return Object.keys(h).reduce((t, e) => ((t[e] = Gt(h[e])), t), {});
};
function Gr(h) {
  const t = h.split('-').map((l) => Number(l)),
    [e, i, n] = t;
  return new Date(e, i - 1, n);
}
function Ur(h, t) {
  return h.setHours(0, 0, 0, 0), t.setHours(0, 0, 0, 0), h.getTime() === t.getTime();
}
var Ut;
class Vr {
  constructor(t) {
    r(this, Ut, void 0);
    a(this, Ut, t.cssPrefix);
    const e = `
      <div class="${s(this, Ut)}cell-editor-container"><input type="text" value="${t.value}"></div>
    `;
    this.element = v(e);
  }
  getGui() {
    return this.element;
  }
  setValue() {}
  parseValue() {}
}
Ut = new WeakMap();
var Et, R, Vt, Ce, Nt, De, vi, Ee, wi, $e, xi;
class we {
  constructor(t, e, i, n, l, c, d) {
    r(this, De);
    r(this, Ee);
    r(this, $e);
    r(this, Et, void 0);
    r(this, R, void 0);
    r(this, Vt, void 0);
    r(this, Ce, void 0);
    r(this, Nt, void 0);
    (this.colIndex = e),
      (this.rowIndex = i),
      (this.width = l),
      a(this, Et, t),
      a(this, R, d),
      a(this, Nt, c ? 'auto' : `${n}px`),
      (this.pinned = c),
      o(this, De, vi).call(this);
  }
  render() {
    const t =
        /* HTML */
        `
          <div
            class="${s(this, R).cssPrefix}cell ${s(this, R).draggable && !this.pinned
              ? `${s(this, R).cssPrefix}cell-draggable`
              : ''} ${this.pinned === 'left' ? `${s(this, R).cssPrefix}cell-pinned-left` : ''} ${this.pinned === 'right'
              ? `${s(this, R).cssPrefix}cell-pinned-right`
              : ''}"
            role="gridcell"
            ref="cell"
            style="left: ${s(this, Nt)}; width: ${this.width}px;"
          ></div>
        `,
      e = v(t);
    return e.appendChild(s(this, Vt).getGui()), e;
  }
}
(Et = new WeakMap()),
  (R = new WeakMap()),
  (Vt = new WeakMap()),
  (Ce = new WeakMap()),
  (Nt = new WeakMap()),
  (De = new WeakSet()),
  (vi = function () {
    const e = s(this, R).columns[this.colIndex].cellRenderer,
      i = o(this, Ee, wi).call(this),
      n = o(this, $e, xi).call(this);
    a(this, Vt, new e(i)), a(this, Ce, new Vr(n));
  }),
  (Ee = new WeakSet()),
  (wi = function () {
    return {
      value: s(this, Et),
      cssPrefix: s(this, R).cssPrefix,
    };
  }),
  ($e = new WeakSet()),
  (xi = function () {
    return {
      value: s(this, Et),
      cssPrefix: s(this, R).cssPrefix,
    };
  });
class Nr {
  constructor(t) {
    this.element = v(`<span>${t.value}</span>`);
  }
  getGui() {
    return this.element;
  }
}
var Bt;
class Br {
  constructor(t) {
    r(this, Bt, void 0);
    a(this, Bt, t.cssPrefix);
    const e = `<div class="${s(this, Bt)}header-cell-value">${t.value}</div>`;
    this.element = v(e);
  }
  getGui() {
    return this.element;
  }
}
Bt = new WeakMap();
class jr {}
class b {
  constructor(t) {
    this.action = t;
  }
}
var rt, C, jt, ye, Si, Fe, Pi, Re, Ci;
class Qr extends jr {
  constructor(e, i, n, l, c) {
    super();
    r(this, ye);
    r(this, Fe);
    r(this, Re);
    r(this, rt, void 0);
    r(this, C, void 0);
    r(this, jt, void 0);
    (this.PubSub = l), a(this, C, n);
    const d = `
      <div class="${s(this, C)}header-sorter">
        <div
          ref="sorterArrow"
          class="${s(this, C)}header-sorter-arrow ${s(this, C)}header-sorter-arrow-${i}"
        >
          <svg 
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            class="${s(this, C)}header-sorter-arrow-svg"
          >
            <path
              stroke="currentColor"
              stroke-linecap="round"
              stroke-linejoin="round"
              d="M7.25 9.75 12 3m0 0 4.75 6.75M12 3v18"
            />
          </svg>
        </div>
        <div ref="sorterIndex" class="${s(this, C)}header-sorter-index"></div>
      </div>
    `;
    a(this, rt, e),
      a(this, jt, c),
      (this.element = v(d)),
      (this.arrowElement = this.element.querySelector('[ref="sorterArrow"]')),
      this.element.addEventListener('click', (f) => {
        const u = s(this, jt) === 'shift' ? f.shiftKey : f.ctrlKey;
        window.getSelection().removeAllRanges(), this.sort(u);
      }),
      o(this, ye, Si).call(this);
  }
  destroy() {
    throw new Error('Method not implemented.');
  }
  getGui() {
    return this.element;
  }
  sort(e) {
    const i = 'command:sorting:start';
    this.PubSub.publish(i, {
      caller: this,
      context: new b(i),
      field: s(this, rt),
      isMultiSort: e,
    });
  }
}
(rt = new WeakMap()),
  (C = new WeakMap()),
  (jt = new WeakMap()),
  (ye = new WeakSet()),
  (Si = function () {
    this.PubSub.subscribe('command:fetch:sort-start', (e) => {
      const { field: i, order: n, isMultiSort: l, activeSorters: c } = e.data;
      s(this, rt) === i && o(this, Fe, Pi).call(this, n, l), o(this, Re, Ci).call(this, c);
    });
  }),
  (Fe = new WeakSet()),
  (Pi = function (e, i) {
    i ||
      document.querySelectorAll('[ref="sorterArrow"]').forEach((l) => {
        l.classList.remove(`${s(this, C)}header-sorter-arrow-asc`, `${s(this, C)}header-sorter-arrow-desc`);
      }),
      e === '' &&
        i &&
        this.arrowElement.classList.remove(
          `${s(this, C)}header-sorter-arrow-asc`,
          `${s(this, C)}header-sorter-arrow-desc`
        ),
      this.arrowElement.classList.add(`${s(this, C)}header-sorter-arrow-${e}`);
  }),
  (Re = new WeakSet()),
  (Ci = function (e) {
    const i = this.element.querySelector('[ref="sorterIndex"]'),
      n = e.find((l) => l.field === s(this, rt));
    if (e.length > 1 && n) {
      const l = n.sortIndex + 1;
      i.textContent = l.toString();
    } else i.textContent = '';
  });
class Xr {}
var $t, yt, Le, Di;
class Kr extends Xr {
  constructor(e, i, n) {
    super();
    r(this, Le);
    r(this, $t, void 0);
    r(this, yt, void 0);
    (this.PubSub = n), a(this, $t, e), a(this, yt, i.pinned === 'right');
    const l = `<div class="${s(this, $t)}header-resizer ${
      s(this, yt) ? s(this, $t) + 'header-resizer-left' : ''
    }"></div>`;
    (this.element = v(l)), this.element.addEventListener('mousedown', o(this, Le, Di).bind(this));
  }
  getGui() {
    return this.element;
  }
  destroy() {}
}
($t = new WeakMap()),
  (yt = new WeakMap()),
  (Le = new WeakSet()),
  (Di = function (e) {
    e.stopPropagation(),
      this.PubSub.publish('info:resizing:mousedown', {
        target: e.target,
        isPinnedRight: s(this, yt),
        context: new b('info:resizing:mousedown'),
      });
  });
var q, Ie, Ei;
class _r {
  constructor(t, e, i, n) {
    r(this, Ie);
    r(this, q, void 0);
    (this.PubSub = n), a(this, q, t), (this.colDef = e), (this.colIndex = i);
    const l = `<button class="${s(this, q)}header-menu-toggler">
      <svg xmlns="http://www.w3.org/2000/svg" width="16px" height="100%" viewBox="0 0 16 16" fill="currenColor" class="${s(
        this,
        q
      )}header-menu-toggler-icon">
        <path stroke="currentColor" fill="currentColor" d="M9.5 13a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0z"/>
      </svg>
    </button>
    `,
      c = `<div class="${s(this, q)}header-menu-dropdown">
      ${
        this.colDef.pinned
          ? `<a href="#" class="${s(this, q)}header-menu-dropdown-item" ref="unPinColTrigger">Unpin</a>`
          : ''
      }
      ${
        this.colDef.pinned === 'right' || !this.colDef.pinned
          ? `<a href="#" class="${s(this, q)}header-menu-dropdown-item" ref="pinLeftPinColTrigger">Pin to the left</a>`
          : ''
      }
      ${
        this.colDef.pinned === 'left' || !this.colDef.pinned
          ? `<a href="#" class="${s(
              this,
              q
            )}header-menu-dropdown-item" ref="pinRightPinColTrigger">Pin to the right</a>`
          : ''
      }
    </div>`;
    (this.toggler = v(l)), (this.dropdown = v(c)), o(this, Ie, Ei).call(this);
  }
  getGui() {
    return this.toggler;
  }
  getDropdownGui() {
    return this.dropdown;
  }
  destroy() {}
}
(q = new WeakMap()),
  (Ie = new WeakSet()),
  (Ei = function () {
    this.toggler.addEventListener('click', () => {
      this.dropdown.classList.toggle('active');
    });
    const t = this.dropdown.querySelector('[ref="unPinColTrigger"]'),
      e = this.dropdown.querySelector('[ref="pinLeftPinColTrigger"]'),
      i = this.dropdown.querySelector('[ref="pinRightPinColTrigger"]');
    t == null ||
      t.addEventListener('click', (n) => {
        n.preventDefault(),
          this.PubSub.publish('command:pin-column:start', {
            pin: !1,
            colIndex: this.colIndex,
            context: new b('command:pin-column:start'),
          });
      }),
      e == null ||
        e.addEventListener('click', (n) => {
          n.preventDefault(),
            this.PubSub.publish('command:pin-column:start', {
              pin: 'left',
              colIndex: this.colIndex,
              context: new b('command:pin-column:start'),
            });
        }),
      i == null ||
        i.addEventListener('click', (n) => {
          n.preventDefault(),
            this.PubSub.publish('command:pin-column:start', {
              pin: 'right',
              colIndex: this.colIndex,
              context: new b('command:pin-column:start'),
            });
        }),
      document.addEventListener('click', (n) => {
        this.toggler.contains(n.target) || this.dropdown.classList.remove('active');
      }),
      document.addEventListener('keydown', (n) => {
        n.key === 'Escape' && this.dropdown.classList.remove('active');
      });
  });
var Qt, Xt, X, nt, at, ot, Kt, w, _t, Ft, Me, $i, Oe, yi, Ae, Fi, ke, Ri, Te, Li;
class mi extends we {
  constructor(e, i, n, l, c, d, f, u, g) {
    super(e, n, i, l, c, d, f);
    r(this, Me);
    r(this, Oe);
    r(this, Ae);
    r(this, ke);
    r(this, Te);
    r(this, Qt, void 0);
    r(this, Xt, void 0);
    r(this, X, void 0);
    r(this, nt, void 0);
    r(this, at, void 0);
    r(this, ot, void 0);
    r(this, Kt, void 0);
    r(this, w, void 0);
    r(this, _t, void 0);
    r(this, Ft, void 0);
    (this.PubSub = g),
      a(this, X, null),
      a(this, nt, null),
      a(this, at, null),
      a(this, ot, null),
      a(this, Qt, e),
      a(this, Xt, d ? 'auto' : `${l}px`),
      (this.width = c),
      (this.pinned = d),
      (this.colIndex = i),
      (this.rowIndex = n),
      a(this, w, f),
      a(this, _t, u),
      a(this, Ft, !1),
      o(this, Me, $i).call(this);
  }
  render() {
    const e = s(this, w).columns.some((d) => d.filter),
      i =
        /* HTML */
        `<div
          class="${s(this, w).cssPrefix}header-cell ${s(this, w).draggable && !this.pinned
            ? `${s(this, w).cssPrefix}header-cell-draggable`
            : ''} ${this.pinned === 'left' ? `${s(this, w).cssPrefix}header-cell-pinned-left` : ''} ${this.pinned ===
          'right'
            ? `${s(this, w).cssPrefix}header-cell-pinned-right`
            : ''} ${e ? `${s(this, w).cssPrefix}header-cell-filterable` : ''}"
          role="columnheader"
          ref="headerCell"
          style="left: ${s(this, Xt)}; width: ${this.width}px;"
        >
          <div ref="headerCellMain" class="${s(this, w).cssPrefix}header-cell-main">
            <div ref="headerValueAndSorter" class="${s(this, w).cssPrefix}header-value-and-sorter"></div>
          </div>
        </div>`,
      n = v(i),
      l = n.querySelector('[ref="headerCellMain"]'),
      c = n.querySelector('[ref="headerValueAndSorter"]');
    return (
      c.appendChild(s(this, Kt).getGui()),
      s(this, nt) && c.appendChild(s(this, nt).getGui()),
      s(this, X) && n.appendChild(s(this, X).getGui()),
      l.appendChild(s(this, ot).getGui()),
      o(this, Te, Li).call(this, n),
      s(this, at) && n.appendChild(s(this, at).getGui()),
      n
    );
  }
}
(Qt = new WeakMap()),
  (Xt = new WeakMap()),
  (X = new WeakMap()),
  (nt = new WeakMap()),
  (at = new WeakMap()),
  (ot = new WeakMap()),
  (Kt = new WeakMap()),
  (w = new WeakMap()),
  (_t = new WeakMap()),
  (Ft = new WeakMap()),
  (Me = new WeakSet()),
  ($i = function () {
    const e = s(this, w).columns[this.colIndex],
      i = s(this, w).cssPrefix,
      { field: n, filter: l, sort: c, resize: d } = e,
      f = s(this, _t);
    if ((a(this, Kt, new Br(o(this, Oe, yi).call(this))), a(this, ot, new _r(i, e, this.colIndex, this.PubSub)), l)) {
      const u = e.filterRenderer,
        g = o(this, Ae, Fi).call(this, e);
      a(this, X, new u(g));
    }
    c && a(this, nt, new Qr(n, f, i, this.PubSub, s(this, w).multiSortKey)),
      d && a(this, at, new Kr(i, e, this.PubSub));
  }),
  (Oe = new WeakSet()),
  (yi = function () {
    return {
      value: s(this, Qt),
      cssPrefix: s(this, w).cssPrefix,
    };
  }),
  (Ae = new WeakSet()),
  (Fi = function (e) {
    const i = e.field,
      { method: n, debounceTime: l, listOptions: c } = e.filterOptions;
    return {
      colDef: e,
      field: i,
      method: n,
      debounceTime: l,
      cssPrefix: s(this, w).cssPrefix,
      listOptions: c,
      filterChanged: () => o(this, ke, Ri).call(this),
    };
  }),
  (ke = new WeakSet()),
  (Ri = function () {
    const i = s(this, w).columns[this.colIndex].field,
      n = s(this, X),
      l = new b('info:filtering:header-filter-changed'),
      c = n.getType(),
      d = n.getState(),
      f = n.isActive(),
      u = n.getFilterFn(),
      g = {
        context: l,
        field: i,
        type: c,
        state: d,
        isActive: f,
        filterFn: u,
      };
    this.PubSub.publish('info:filtering:header-filter-changed', g);
  }),
  (Te = new WeakSet()),
  (Li = function (e) {
    var i;
    s(this, Ft) || (e.appendChild((i = s(this, ot)) == null ? void 0 : i.getDropdownGui()), a(this, Ft, !0));
  });
class Ks {}
var ht;
class Jr extends Ks {
  constructor(e) {
    super();
    r(this, ht, void 0);
    (this.params = e), a(this, ht, e.cssPrefix);
    const i =
      /* HTLM */
      `
      <div class="${s(this, ht)}header-filter">
        <input type="text" class="${s(this, ht)}header-filter-input">
      </div>
    `;
    (this.element = v(i)),
      (this.input = this.element.querySelector(`.${s(this, ht)}header-filter-input`)),
      this.attachUiEvents(e);
  }
  getGui() {
    return this.element;
  }
  getType() {
    return 'text';
  }
  getState() {
    const e = this.input ? this.input.value : null;
    return {
      method: this.params.method,
      searchTerm: e,
    };
  }
  isActive() {
    return !!(this.input ? this.input.value : null);
  }
  getFilterFn() {
    const e = this.params.colDef.filterOptions;
    return (i, n) => {
      let l = i.searchTerm;
      const c = i.method,
        d = e.caseSensitive;
      switch (((n = d ? n : n.toString().toLowerCase()), (l = d ? l : l.toString().toLowerCase()), c)) {
        case 'includes':
          return n.includes(l);
        default:
          return !1;
      }
    };
  }
  attachUiEvents(e) {
    if (this.input) {
      const i = gi(() => e.filterChanged(), e.debounceTime);
      this.input.addEventListener('keyup', () => i());
    }
  }
  destroy() {}
}
ht = new WeakMap();
var lt;
class Cn extends Ks {
  constructor(e) {
    super();
    r(this, lt, void 0);
    (this.params = e), a(this, lt, e.cssPrefix);
    const i =
      /* HTML */
      `
        <div class="${s(this, lt)}header-filter">
          <input type="number" class="${s(this, lt)}header-filter-input" />
        </div>
      `;
    (this.element = v(i)),
      (this.input = this.element.querySelector(`.${s(this, lt)}header-filter-input`)),
      this.attachUiEvents(e);
  }
  getGui() {
    return this.element;
  }
  getType() {
    return 'number';
  }
  getState() {
    const e = this.input.value;
    return {
      method: this.params.method,
      searchTerm: e,
    };
  }
  isActive() {
    return !!this.input.value;
  }
  getFilterFn() {
    return (e, i) => {
      const n = e.method,
        l = Number(e.searchTerm),
        c = Number(i);
      switch (n) {
        case 'equals':
          return c === l;
        default:
          return !1;
      }
    };
  }
  attachUiEvents(e) {
    if (this.input) {
      const i = gi(() => e.filterChanged(), e.debounceTime);
      this.input.addEventListener('keyup', () => i());
    }
  }
  destroy() {}
}
lt = new WeakMap();
var ct;
class Dn extends Ks {
  constructor(e) {
    super();
    r(this, ct, void 0);
    (this.params = e), a(this, ct, e.cssPrefix);
    const i =
      /* HTML */
      `
        <div class="${s(this, ct)}header-filter">
          <input type="date" class="${s(this, ct)}header-filter-input" />
        </div>
      `;
    (this.element = v(i)),
      (this.input = this.element.querySelector(`.${s(this, ct)}header-filter-input`)),
      this.attachUiEvents(e);
  }
  getGui() {
    return this.element;
  }
  getType() {
    return 'date';
  }
  getState() {
    const e = this.input.value;
    return {
      method: this.params.method,
      searchTerm: e,
    };
  }
  isActive() {
    return !!this.input.value;
  }
  getFilterFn() {
    const e = this.params.colDef.filterOptions;
    return (i, n) => {
      const l = e.dateParserFn,
        c = i.method,
        d = Gr(i.searchTerm),
        f = typeof n == 'string' ? l(n) : n;
      switch (c) {
        case 'equals':
          return Ur(d, f);
        default:
          return !1;
      }
    };
  }
  attachUiEvents(e) {
    if (this.input) {
      const i = gi(() => e.filterChanged(), e.debounceTime);
      this.input.addEventListener('change', () => i());
    }
  }
  destroy() {}
}
ct = new WeakMap();
var dt, qe, Ii;
class En extends Ks {
  constructor(e) {
    super();
    r(this, qe);
    r(this, dt, void 0);
    (this.params = e), a(this, dt, e.cssPrefix);
    const i =
      /* HTML */
      `
        <div class="${s(this, dt)}header-filter">
          <select class="${s(this, dt)}header-filter-select">
            <option disabled selected value></option>
          </select>
        </div>
      `;
    (this.element = v(i)),
      (this.select = this.element.querySelector(`.${s(this, dt)}header-filter-select`)),
      o(this, qe, Ii).call(this, e),
      this.attachUiEvents(e);
  }
  getGui() {
    return this.element;
  }
  getType() {
    return 'select';
  }
  getState() {
    return {
      method: this.params.method,
      searchTerm: this.select.value,
    };
  }
  isActive() {
    return !!this.select.value;
  }
  getFilterFn() {
    return (e, i) => {
      const n = e.method;
      switch (((i = i.toString().toLowerCase()), n)) {
        case 'includes':
          return i.includes(e.searchTerm);
        case 'equals':
          return i === e.searchTerm;
        default:
          return !1;
      }
    };
  }
  attachUiEvents(e) {
    this.select &&
      this.select.addEventListener('change', () => {
        e.filterChanged();
      });
  }
  destroy() {}
}
(dt = new WeakMap()),
  (qe = new WeakSet()),
  (Ii = function (e) {
    if (this.select) {
      const i = e.listOptions,
        n = document.createDocumentFragment();
      i.forEach((l) => {
        const c = document.createElement('option');
        (c.label = l.label), (c.value = l.value), n.appendChild(c);
      }),
        this.select.appendChild(n);
    }
  });
var W;
class Mi {
  constructor(t, e, i) {
    r(this, W, void 0);
    (this.index = t), (this.cells = e), a(this, W, i);
    const n =
      /* HTML */
      `<div class="${i.cssPrefix}row" role="row" ref="row" style="height: ${i.rowHeight}px;"></div>`;
    this.element = v(n);
  }
  render() {
    const t = document.createDocumentFragment(),
      e = this.cells.filter((d) => d.pinned === 'left').reduce((d, f) => d + f.width, 0),
      i = this.cells.filter((d) => d.pinned === 'right').reduce((d, f) => d + f.width, 0),
      n = v(
        /* HTML */
        `<div
          class="${s(this, W).cssPrefix}pinned-cells-wrapper ${s(this, W).cssPrefix}pinned-cells-wrapper-left"
        ></div>`
      ),
      l = v(
        /* HTML */
        `<div
          class="${s(this, W).cssPrefix}main-cells-wrapper"
          style="left: ${e}px; width: calc(100% - ${e + i}px);"
          ref="rowMainCellsWrapper"
        ></div>`
      ),
      c = v(
        /* HTML */
        `<div
          class="${s(this, W).cssPrefix}pinned-cells-wrapper ${s(this, W).cssPrefix}pinned-cells-wrapper-right"
        ></div>`
      );
    return (
      t.appendChild(n),
      t.appendChild(l),
      t.appendChild(c),
      this.cells.filter((d) => d.pinned === 'left').forEach((d) => n.appendChild(d.render())),
      this.cells.filter((d) => !d.pinned).forEach((d) => l.appendChild(d.render())),
      this.cells.filter((d) => d.pinned === 'right').forEach((d) => c.appendChild(d.render())),
      this.element.appendChild(t),
      this.element
    );
  }
}
W = new WeakMap();
var K;
class Yr extends Mi {
  constructor(e, i, n) {
    super(e, i, n);
    r(this, K, void 0);
    (this.index = e), (this.cells = i), a(this, K, n);
    const l = s(this, K).columns.some((d) => d.filter) ? n.headerHeight + 15 : n.headerHeight,
      c =
        /* HTML */
        `<div class="${n.cssPrefix}header-row" role="row" ref="headerRow" style="height: ${l}px"></div>`;
    this.element = v(c);
  }
  render() {
    const e = document.createDocumentFragment(),
      i = this.cells.filter((f) => f.pinned === 'left').reduce((f, u) => f + u.width, 0),
      n = this.cells.filter((f) => f.pinned === 'right').reduce((f, u) => f + u.width, 0),
      l = v(
        /* HTML */
        `<div ref="headerLeftCellsWrapper" class="${s(this, K).cssPrefix}header-left-cells-wrapper"></div>`
      ),
      c = v(
        /* HTML */
        `<div
          ref="headerMainCellsWrapper"
          class="${s(this, K).cssPrefix}header-main-cells-wrapper"
          style="left: ${i}px; width: calc(100% - ${i + n}px)"
        ></div>`
      ),
      d = v(
        /* HTML */
        `<div ref="headerRightCellsWrapper" class="${s(this, K).cssPrefix}header-right-cells-wrapper"></div>`
      );
    return (
      e.appendChild(l),
      e.appendChild(c),
      e.appendChild(d),
      this.cells.filter((f) => f.pinned === 'left').forEach((f) => l.appendChild(f.render())),
      this.cells.filter((f) => !f.pinned).forEach((f) => c.appendChild(f.render())),
      this.cells.filter((f) => f.pinned === 'right').forEach((f) => d.appendChild(f.render())),
      this.element.appendChild(e),
      this.element
    );
  }
}
K = new WeakMap();
var P;
class Zr {
  constructor(t, e, i) {
    r(this, P, void 0);
    (this.PubSub = i), (this.options = t), a(this, P, e);
    const n =
      /* HTML */
      `<div class="${s(this, P)}pagination">
        <div class="${s(this, P)}pagination-content">
          <button class="${s(this, P)}pagination-button" ref="data-den-pagination-first-button">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="${s(this, P)}pagination-button-svg"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="m18.75 4.5-7.5 7.5 7.5 7.5m-6-15L5.25 12l7.5 7.5"
              />
            </svg>
          </button>
          <button class="${s(this, P)}pagination-button" ref="data-den-pagination-prev-button">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="${s(this, P)}pagination-button-svg"
            >
              <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
            </svg>
          </button>
          <div class="${s(this, P)}pagination-info" ref="data-den-pagination-info">
            1-10 ${this.options.ofText || 'of'} 100
          </div>
          <button class="${s(this, P)}pagination-button" ref="data-den-pagination-next-button">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="${s(this, P)}pagination-button-svg"
            >
              <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
            </svg>
          </button>
          <button class="${s(this, P)}pagination-button" ref="data-den-pagination-last-button">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="${s(this, P)}pagination-button-svg"
            >
              <path stroke-linecap="round" stroke-linejoin="round" d="m5.25 4.5 7.5 7.5-7.5 7.5m6-15 7.5 7.5-7.5 7.5" />
            </svg>
          </button>
        </div>
      </div>`;
    (this.element = v(n)),
      (this.buttons = {
        first: this.element.querySelector('[ref=data-den-pagination-first-button]'),
        prev: this.element.querySelector('[ref=data-den-pagination-prev-button]'),
        next: this.element.querySelector('[ref=data-den-pagination-next-button]'),
        last: this.element.querySelector('[ref=data-den-pagination-last-button]'),
      }),
      this.attachUiEvents(),
      this.subscribeToEvents();
  }
  getGui() {
    return this.element;
  }
  attachUiEvents() {
    this.buttons.first.addEventListener('click', () => {
      this.PubSub.publish('command:pagination:load-first-page:start', {
        context: new b('command:pagination:load-first-page:start'),
      });
    }),
      this.buttons.prev.addEventListener('click', () => {
        this.PubSub.publish('command:pagination:load-prev-page:start', {
          context: new b('command:pagination:load-prev-page:start'),
        });
      }),
      this.buttons.next.addEventListener('click', () => {
        this.PubSub.publish('command:pagination:load-next-page:start', {
          context: new b('command:pagination:load-next-page:start'),
        });
      }),
      this.buttons.last.addEventListener('click', () => {
        this.PubSub.publish('command:pagination:load-last-page:start', {
          context: new b('command:pagination:load-last-page:start'),
        });
      });
  }
  destroy() {
    var t;
    (this.element.innerHTML = ''), (t = document.querySelector('.data-den-pagination')) == null || t.remove();
  }
  subscribeToEvents() {
    this.PubSub.subscribe('info:pagination:info-change:done', (t) => {
      this.updateInfo(t.data.firstRowIndex, t.data.lastRowIndex, t.data.allTotalRows),
        this.updateButtonsState(t.data.firstRowIndex, t.data.lastRowIndex, t.data.allTotalRows);
    });
  }
  updateButtonsState(t, e, i) {
    Object.values(this.buttons).forEach((n) => (n.disabled = !1)),
      t === 0 && ((this.buttons.first.disabled = !0), (this.buttons.prev.disabled = !0)),
      e === i && ((this.buttons.next.disabled = !0), (this.buttons.last.disabled = !0));
  }
  updateInfo(t, e, i) {
    const n = this.element.querySelector('[ref=data-den-pagination-info');
    n.innerHTML = `${t + 1}-${e} ${this.options.ofText || 'of'} ${i}`;
  }
  updatePageSize(t) {
    this.PubSub.publish('info:pagination:page-size-change:done', {
      pageSize: t,
      context: new b('info:pagination:page-size-change:done'),
    });
  }
}
P = new WeakMap();
const Oi = (h) => h.filter((t) => t.pinned === 'left'),
  _s = (h) => h.filter((t) => !t.pinned),
  Zs = (h) => h.filter((t) => t.pinned === 'right'),
  bi = (h) => Ai(h).map((t) => h[t]),
  Ai = (h) => _s(h).map((t) => h.indexOf(t)),
  tn = (h) => _s(h).map((t, e) => e),
  ti = (h) => [...Oi(h).map((t) => h.indexOf(t)), ..._s(h).map((t) => h.indexOf(t)), ...Zs(h).map((t) => h.indexOf(t))];
class en {
  constructor(t) {
    (this.data = {}), (this.data = { ...this.data, ...t });
  }
  getValue(t) {
    return this.data[t];
  }
}
var ut;
class Wt {
  static on(t, e) {
    s(this, ut)[t] || (s(this, ut)[t] = []), s(this, ut)[t].push(e);
  }
  static triggerEvent(t, e) {
    var n;
    const i = {
      name: t,
      preventDefault: () => {
        i.defaultPrevented = !0;
      },
      defaultPrevented: !1,
      ...e,
    };
    return (
      (n = s(this, ut)[t]) == null ||
        n.forEach((l) => {
          l(i);
        }),
      i
    );
  }
}
(ut = new WeakMap()), r(Wt, ut, {});
var I,
  p,
  L,
  Jt,
  Rt,
  Yt,
  ft,
  Lt,
  Zt,
  ei,
  te,
  si,
  He,
  ki,
  ze,
  Ti,
  We,
  qi,
  Ge,
  Hi,
  Ue,
  zi,
  Ve,
  Wi,
  ee,
  ii,
  It,
  xe,
  Ne,
  Gi,
  Be,
  Ui,
  je,
  Vi,
  Qe,
  Ni,
  Xe,
  Bi,
  Ke,
  ji;
class sn {
  constructor(t, e, i) {
    r(this, Zt);
    r(this, te);
    r(this, He);
    r(this, ze);
    r(this, We);
    r(this, Ge);
    r(this, Ue);
    r(this, Ve);
    r(this, ee);
    r(this, It);
    r(this, Ne);
    r(this, Be);
    r(this, je);
    r(this, Qe);
    r(this, Xe);
    r(this, Ke);
    r(this, I, void 0);
    r(this, p, void 0);
    r(this, L, void 0);
    r(this, Jt, void 0);
    r(this, Rt, void 0);
    r(this, Yt, void 0);
    r(this, ft, []);
    r(this, Lt, null);
    (this.PubSub = i),
      a(this, I, t),
      a(this, p, e),
      e.pagination && a(this, Lt, new Zr(e.paginationOptions, e.cssPrefix, this.PubSub)),
      o(this, Zt, ei).call(this),
      o(this, Qe, Ni).call(this),
      o(this, Xe, Bi).call(this),
      o(this, ee, ii).call(this);
  }
  renderTable() {
    const t = o(this, Ne, Gi).call(this),
      e = document.createElement('div');
    e.setAttribute('ref', 'gridMain'),
      e.classList.add(`${s(this, p).cssPrefix}grid-main`),
      e.appendChild(o(this, Be, Ui).call(this)),
      e.appendChild(o(this, je, Vi).call(this)),
      t.appendChild(e),
      s(this, Lt) && t.appendChild(s(this, Lt).getGui()),
      s(this, I).appendChild(t),
      o(this, It, xe).call(this);
  }
  rerenderTable() {
    (s(this, I).innerHTML = ''),
      o(this, Zt, ei).call(this),
      o(this, ee, ii).call(this),
      this.PubSub.publish('command:rerendering:done', {
        caller: this,
        context: new b('command:rerendering:done'),
      }),
      this.PubSub.publish('command:rerendering:done', {
        caller: this,
        context: new b('command:rerendering:done'),
      });
  }
}
(I = new WeakMap()),
  (p = new WeakMap()),
  (L = new WeakMap()),
  (Jt = new WeakMap()),
  (Rt = new WeakMap()),
  (Yt = new WeakMap()),
  (ft = new WeakMap()),
  (Lt = new WeakMap()),
  (Zt = new WeakSet()),
  (ei = function () {
    a(this, L, bi(s(this, p).columns)),
      a(this, Jt, bi(s(this, p).columns)),
      a(this, Rt, Ai(s(this, p).columns)),
      a(this, Yt, o(this, ze, Ti).call(this, s(this, p).columns, '')),
      this.renderTable();
  }),
  (te = new WeakSet()),
  (si = function (t, e, i) {
    return t.map((n) => {
      const l = n.headerName,
        c = 0,
        d = n.width || 120,
        f = s(this, p)
          .columns.map((u) => u.field)
          .indexOf(n.field);
      return new mi(l, f, e, c, d, n.pinned, s(this, p), i, this.PubSub);
    });
  }),
  (He = new WeakSet()),
  (ki = function (t, e, i) {
    return t.map((n, l) => {
      const c = n.headerName,
        d = s(this, L)
          .slice(0, l)
          .reduce((g, x) => g + (x.width || 120), 0),
        f = s(this, L)[l].width || 120,
        u = s(this, p)
          .columns.map((g) => g.field)
          .indexOf(n.field);
      return new mi(c, u, e, d, f, n.pinned, s(this, p), i, this.PubSub);
    });
  }),
  (ze = new WeakSet()),
  (Ti = function (t, e) {
    const n = o(this, te, si).call(this, Oi(t), 0, e),
      l = o(this, He, ki).call(this, _s(t), 0, e),
      c = o(this, te, si).call(this, Zs(t).reverse(), 0, e),
      d = [...n, ...l, ...c];
    return new Yr(0, d, s(this, p));
  }),
  (We = new WeakSet()),
  (qi = function (t, e, i, n) {
    const l = s(this, p).columns.find((f) => f.field === t);
    if (l.pinned !== 'left') return;
    const c = 0,
      d = s(this, p).columns[i].width || 120;
    return new we(e, i, n, c, d, l.pinned, s(this, p));
  }),
  (Ge = new WeakSet()),
  (Hi = function (t, e, i, n) {
    const l = s(this, p).columns.find((u) => u.field === t);
    if (l.pinned) return;
    const c = s(this, L).findIndex((u) => u.field === t),
      d = s(this, L)
        .slice(0, c)
        .reduce((u, g) => u + (g.width || 120), 0),
      f = s(this, L)[c].width || 120;
    return new we(e, i, n, d, f, l.pinned, s(this, p));
  }),
  (Ue = new WeakSet()),
  (zi = function (t, e, i, n) {
    const l = s(this, p).columns.find((u) => u.field === t);
    if (l.pinned !== 'right') return;
    const c = s(this, p)
        .columns.filter((u) => u.pinned === 'right')
        .map((u) => u.field)
        .indexOf(t),
      d = 0,
      f = Zs(s(this, p).columns)[c].width || 120;
    return new we(e, i, n, d, f, l.pinned, s(this, p));
  }),
  (Ve = new WeakSet()),
  (Wi = function (t) {
    return t.map((e, i) => {
      const n = Object.entries(e).map(([u, g], x) => o(this, We, qi).call(this, u, g, x, i)),
        l = Object.entries(e);
      l.sort(([u], [g]) => {
        const x = s(this, L).findIndex((Q) => Q.field === u),
          be = s(this, L).findIndex((Q) => Q.field === g);
        return x - be;
      });
      const c = l.map(([u, g], x) => o(this, Ge, Hi).call(this, u, g, x, i)),
        d = Object.entries(e)
          .reverse()
          .map(([u, g], x) => o(this, Ue, zi).call(this, u, g, x, i)),
        f = [...n, ...c, ...d].filter((u) => u !== void 0);
      return new Mi(i, f, s(this, p));
    });
  }),
  (ee = new WeakSet()),
  (ii = function () {
    this.PubSub.publish('command:fetch:start', {
      caller: this,
      context: new b('command:fetch:start'),
    });
  }),
  (It = new WeakSet()),
  (xe = function () {
    const t = s(this, I).querySelector(`.${s(this, p).cssPrefix}header`),
      e = s(this, I).querySelector(`.${s(this, p).cssPrefix}grid-rows`),
      i = s(this, I).querySelector('[ref=headerMainCellsWrapper]'),
      n = s(this, I).querySelectorAll('[ref=rowMainCellsWrapper]'),
      l = s(this, p).columns.reduce((u, g) => u + (g.width || 120), 0),
      c = s(this, p)
        .columns.filter((u) => u.pinned === 'left')
        .reduce((u, g) => u + (g.width || 120), 0),
      d = s(this, p)
        .columns.filter((u) => !u.pinned)
        .reduce((u, g) => u + (g.width || 120), 0),
      f = s(this, p).rowHeight * s(this, ft).length;
    (t.style.width = `${l}px`),
      (e.style.width = `${l}px`),
      (e.style.height = `${f}px`),
      (i.style.left = `${c}px`),
      (i.style.width = `${d}px`),
      n &&
        n.forEach((u) => {
          (u.style.left = `${c}px`), (u.style.width = `${d}px`);
        });
  }),
  (Ne = new WeakSet()),
  (Gi = function () {
    const t = document.createElement('div');
    return t.classList.add(`${s(this, p).cssPrefix}grid`), t;
  }),
  (Be = new WeakSet()),
  (Ui = function () {
    const t = document.createElement('div');
    t.classList.add(`${s(this, p).cssPrefix}header`), t.setAttribute('role', 'rowgroup');
    const e = s(this, Yt).render();
    return t.appendChild(e), t;
  }),
  (je = new WeakSet()),
  (Vi = function () {
    const t = document.createElement('div');
    t.classList.add(`${s(this, p).cssPrefix}grid-rows`), t.setAttribute('role', 'rowgroup');
    const e = document.createDocumentFragment();
    return s(this, ft).forEach((i) => e.appendChild(i.render())), t.appendChild(e), t;
  }),
  (Qe = new WeakSet()),
  (Ni = function () {
    this.PubSub.subscribe('info:dragging:columns-reorder:done', (t) => {
      a(this, Rt, t.data.columnsOrder),
        a(
          this,
          L,
          s(this, Rt).map((e) => s(this, Jt)[e])
        );
    }),
      this.PubSub.subscribe('info:resizing:start', (t) => {
        const e = ti(s(this, p).columns)[t.data.currentColIndex];
        (s(this, p).columns[e].width = t.data.newCurrentColWidth), o(this, It, xe).call(this);
      }),
      this.PubSub.subscribe('command:pin-column:start', (t) => {
        const e = new en({
          pin: s(this, p).columns[t.data.colIndex].pinned,
        });
        if (
          ((s(this, p).columns[t.data.colIndex].pinned = t.data.pin),
          Wt.triggerEvent('pinningStart', {
            pin: t.data.pin,
            colIndex: t.data.colIndex,
            columns: s(this, p).columns,
          }).defaultPrevented)
        ) {
          s(this, p).columns[t.data.colIndex].pinned = e.getValue('pin');
          return;
        }
        this.rerenderTable(),
          Wt.triggerEvent('pinningDone', {
            pin: t.data.pin,
            colIndex: t.data.colIndex,
            columns: s(this, p).columns,
          });
      });
  }),
  (Xe = new WeakSet()),
  (Bi = function () {
    this.PubSub.subscribe('info:fetch:done', (t) => {
      o(this, Ke, ji).call(this, t);
    });
  }),
  (Ke = new WeakSet()),
  (ji = function (t) {
    const { rows: e } = t.data,
      i = document.createDocumentFragment();
    a(this, ft, o(this, Ve, Wi).call(this, e)), s(this, ft).forEach((l) => i.appendChild(l.render()));
    const n = s(this, I).querySelector(`.${s(this, p).cssPrefix}grid-rows`);
    (n.innerHTML = ''), n.appendChild(i), o(this, It, xe).call(this);
  });
var se, $, y, F, _e, Qi, G, it, Je, Xi, Ye, Ki, Ze, _i, ts, Ji, es, Yi;
class rn {
  constructor(t, e) {
    r(this, _e);
    r(this, G);
    r(this, Je);
    r(this, Ye);
    r(this, Ze);
    r(this, ts);
    r(this, es);
    r(this, se, void 0);
    r(this, $, void 0);
    r(this, y, void 0);
    r(this, F, void 0);
    (this.PubSub = e),
      a(this, se, t.paginationOptions),
      a(this, $, 0),
      a(this, F, 0),
      a(this, y, s(this, se).pageSize || 10),
      o(this, _e, Qi).call(this);
  }
}
(se = new WeakMap()),
  ($ = new WeakMap()),
  (y = new WeakMap()),
  (F = new WeakMap()),
  (_e = new WeakSet()),
  (Qi = function () {
    this.PubSub.subscribe('command:pagination:load-first-page:start', () => {
      o(this, Ye, Ki).call(this);
    }),
      this.PubSub.subscribe('command:pagination:load-prev-page:start', () => {
        o(this, Ze, _i).call(this);
      }),
      this.PubSub.subscribe('command:pagination:load-next-page:start', () => {
        o(this, ts, Ji).call(this);
      }),
      this.PubSub.subscribe('command:pagination:load-last-page:start', () => {
        o(this, es, Yi).call(this);
      }),
      this.PubSub.subscribe('info:pagination:page-size-change:done', (t) => {
        a(this, y, t.data.pageSize), a(this, $, 0), o(this, G, it).call(this);
      }),
      this.PubSub.subscribe('info:fetch:done', (t) => {
        s(this, F) || s(this, F) === t.data.rows.length || (a(this, F, t.data.rows.length), o(this, G, it).call(this));
      });
  }),
  (G = new WeakSet()),
  (it = function () {
    const t = s(this, $) * s(this, y),
      e = Math.min(s(this, F), s(this, $) * s(this, y) + s(this, y));
    o(this, Je, Xi).call(this, t, e, s(this, F));
  }),
  (Je = new WeakSet()),
  (Xi = function (t, e, i) {
    this.PubSub.publish('info:pagination:info-change:done', {
      firstRowIndex: t,
      lastRowIndex: e,
      allTotalRows: i,
      pageSize: s(this, y),
      context: new b('info:pagination:info-change:done'),
    });
  }),
  (Ye = new WeakSet()),
  (Ki = function () {
    a(this, $, 0), o(this, G, it).call(this);
  }),
  (Ze = new WeakSet()),
  (_i = function () {
    s(this, $) !== 0 && (Ys(this, $)._--, o(this, G, it).call(this));
  }),
  (ts = new WeakSet()),
  (Ji = function () {
    s(this, $) * s(this, y) + s(this, y) >= s(this, F) || (Ys(this, $)._++, o(this, G, it).call(this));
  }),
  (es = new WeakSet()),
  (Yi = function () {
    s(this, F) / s(this, y) - 1 >= s(this, F) ||
      (a(this, $, Math.ceil(s(this, F) / s(this, y) - 1)), o(this, G, it).call(this));
  });
var M,
  _,
  pt,
  Mt,
  J,
  H,
  U,
  O,
  m,
  V,
  A,
  k,
  S,
  N,
  gt,
  Y,
  mt,
  bt,
  vt,
  ss,
  Zi,
  is,
  tr,
  ie,
  ri,
  re,
  ni,
  ne,
  ai,
  ae,
  oi,
  oe,
  hi,
  rs,
  er,
  ns,
  sr,
  he,
  li,
  le,
  ci,
  as,
  ir,
  os,
  rr,
  hs,
  nr,
  ls,
  ar,
  cs,
  or,
  ds,
  hr,
  us,
  lr,
  fs,
  cr,
  ps,
  dr,
  gs,
  ur,
  ce,
  di,
  ms,
  fr,
  Ot,
  Se,
  de,
  ui,
  At,
  Pe,
  bs,
  pr,
  vs,
  gr,
  ws,
  mr,
  xs,
  br,
  Ss,
  vr;
class nn {
  constructor(t, e, i) {
    r(this, ss);
    r(this, is);
    r(this, ie);
    r(this, re);
    r(this, ne);
    r(this, ae);
    r(this, oe);
    r(this, rs);
    r(this, ns);
    r(this, he);
    r(this, le);
    r(this, as);
    r(this, os);
    r(this, hs);
    r(this, ls);
    r(this, cs);
    r(this, ds);
    r(this, us);
    r(this, fs);
    r(this, ps);
    r(this, gs);
    r(this, ce);
    r(this, ms);
    r(this, Ot);
    r(this, de);
    r(this, At);
    r(this, bs);
    r(this, vs);
    r(this, ws);
    r(this, xs);
    r(this, Ss);
    r(this, M, void 0);
    r(this, _, void 0);
    r(this, pt, void 0);
    r(this, Mt, void 0);
    r(this, J, void 0);
    r(this, H, void 0);
    r(this, U, void 0);
    r(this, O, void 0);
    r(this, m, void 0);
    r(this, V, void 0);
    r(this, A, void 0);
    r(this, k, void 0);
    r(this, S, void 0);
    r(this, N, void 0);
    r(this, gt, void 0);
    r(this, Y, void 0);
    r(this, mt, void 0);
    r(this, bt, void 0);
    r(this, vt, void 0);
    (this.PubSub = i),
      a(this, M, t),
      a(this, _, t.querySelector('[ref="gridMain"]')),
      a(this, pt, e),
      a(this, Mt, !1),
      a(this, J, !1),
      a(this, H, null),
      a(this, U, []),
      a(this, O, -1),
      a(this, m, -1),
      a(this, V, -1),
      a(this, A, []),
      a(this, k, []),
      a(this, S, []),
      a(this, N, []),
      a(this, gt, 0),
      a(this, Y, e.cssPrefix),
      a(this, mt, () => {}),
      a(this, bt, () => {}),
      a(this, vt, () => {}),
      o(this, ss, Zi).call(this),
      o(this, is, tr).call(this);
  }
  init() {
    o(this, le, ci).call(this),
      o(this, ie, ri).call(this),
      this.update(),
      o(this, re, ni).call(this),
      o(this, he, li).call(this),
      o(this, as, ir).call(this),
      a(this, Mt, !0);
  }
  dispose() {
    o(this, ws, mr).call(this), o(this, xs, br).call(this);
  }
  update() {
    const t = [...o(this, rs, er).call(this)];
    a(
      this,
      A,
      s(this, N).map((e) => t[e])
    );
  }
}
(M = new WeakMap()),
  (_ = new WeakMap()),
  (pt = new WeakMap()),
  (Mt = new WeakMap()),
  (J = new WeakMap()),
  (H = new WeakMap()),
  (U = new WeakMap()),
  (O = new WeakMap()),
  (m = new WeakMap()),
  (V = new WeakMap()),
  (A = new WeakMap()),
  (k = new WeakMap()),
  (S = new WeakMap()),
  (N = new WeakMap()),
  (gt = new WeakMap()),
  (Y = new WeakMap()),
  (mt = new WeakMap()),
  (bt = new WeakMap()),
  (vt = new WeakMap()),
  (ss = new WeakSet()),
  (Zi = function () {
    this.PubSub.subscribe('info:fetch:done', () => {
      s(this, Mt) ? this.update() : this.init();
    });
  }),
  (is = new WeakSet()),
  (tr = function () {
    this.PubSub.subscribe('command:rerendering:done', () => {
      o(this, le, ci).call(this),
        o(this, ie, ri).call(this),
        setTimeout(() => {
          a(this, _, s(this, M).querySelector('[ref="gridMain"]')),
            this.update(),
            o(this, re, ni).call(this),
            o(this, he, li).call(this);
        }, 0);
    });
  }),
  (ie = new WeakSet()),
  (ri = function () {
    a(this, H, s(this, M).querySelector('[ref="headerMainCellsWrapper"]')),
      a(this, U, Array.from(s(this, H).querySelectorAll('[ref="headerCell"]')));
  }),
  (re = new WeakSet()),
  (ni = function () {
    a(this, k, [...o(this, oe, hi).call(this)]),
      o(this, ne, ai).call(this),
      a(this, gt, s(this, _).getBoundingClientRect().left + parseFloat(s(this, H).style.left));
  }),
  (ne = new WeakSet()),
  (ai = function () {
    a(this, S, []),
      s(this, k).forEach((t) => {
        t.left !== 'auto' && s(this, S).push(parseFloat(t.left));
      }),
      s(this, S).sort((t, e) => t - e);
  }),
  (ae = new WeakSet()),
  (oi = function (t) {
    return t - s(this, gt) + s(this, _).scrollLeft;
  }),
  (oe = new WeakSet()),
  (hi = function () {
    return s(this, U).map((t) => ({
      left: t.style.left,
      width: parseFloat(t.style.width),
    }));
  }),
  (rs = new WeakSet()),
  (er = function () {
    return s(this, U).map((t, e) => [...o(this, ns, sr).call(this, e)]);
  }),
  (ns = new WeakSet()),
  (sr = function (t) {
    const e = s(this, U)[t],
      i = s(this, M).querySelectorAll('[ref="row"]'),
      n = Array.from(i).map((l) => l.querySelectorAll('.data-den-main-cells-wrapper [ref="cell"]')[t]);
    return [e, ...n];
  }),
  (he = new WeakSet()),
  (li = function () {
    a(this, vt, o(this, os, rr).bind(this)),
      a(this, mt, o(this, cs, or).bind(this)),
      a(this, bt, o(this, ds, hr).bind(this)),
      s(this, H).addEventListener('mousedown', s(this, vt)),
      s(this, M).addEventListener('mousemove', s(this, mt)),
      document.addEventListener('mouseup', s(this, bt));
  }),
  (le = new WeakSet()),
  (ci = function () {
    a(this, N, tn(s(this, pt).columns));
  }),
  (as = new WeakSet()),
  (ir = function () {
    this.PubSub.subscribe('info:resizing:done', () => {
      const t = [...o(this, oe, hi).call(this)];
      a(
        this,
        k,
        s(this, N).map((e) => t[e])
      ),
        o(this, ne, ai).call(this),
        a(this, gt, s(this, _).getBoundingClientRect().left + parseFloat(s(this, H).style.left));
    });
  }),
  (os = new WeakSet()),
  (rr = function (t) {
    t.stopPropagation();
    const e = t.target;
    t.button !== 0 ||
      (e.getAttribute('ref') !== 'headerCell' &&
        e.getAttribute('ref') !== 'headerCellMain' &&
        e.getAttribute('ref') !== 'headerValueAndSorter') ||
      o(this, hs, nr).call(this, o(this, ae, oi).call(this, t.pageX));
  }),
  (hs = new WeakSet()),
  (nr = function (t) {
    o(this, ls, ar).call(this, t), a(this, J, !0), o(this, us, lr).call(this), o(this, fs, cr).call(this);
  }),
  (ls = new WeakSet()),
  (ar = function (t) {
    a(this, O, o(this, ce, di).call(this, s(this, S), t));
  }),
  (cs = new WeakSet()),
  (or = function (t) {
    if (!s(this, J)) return;
    const e = o(this, ae, oi).call(this, t.pageX);
    a(this, m, o(this, ce, di).call(this, s(this, S), e));
    const i = s(this, k)[s(this, O)].width,
      n = o(this, de, ui).call(this, s(this, O));
    if (
      (o(this, Ot, Se).call(this) === 'right' && s(this, S)[s(this, m)] + n > e) ||
      (o(this, Ot, Se).call(this) === 'left' && s(this, S)[s(this, m)] + i < e)
    ) {
      a(this, m, s(this, O));
      return;
    }
    s(this, V) !== s(this, m) && s(this, m) !== -1 && (o(this, ms, fr).call(this), a(this, V, s(this, m)));
  }),
  (ds = new WeakSet()),
  (hr = function () {
    s(this, J) && o(this, bs, pr).call(this);
  }),
  (us = new WeakSet()),
  (lr = function () {
    s(this, A).forEach((t) => {
      t.forEach((e) => {
        e.classList.add(`${s(this, Y)}dragging`);
      });
    });
  }),
  (fs = new WeakSet()),
  (cr = function () {
    s(this, M).children[0].classList.add(`${s(this, pt).cssPrefix}dragging`),
      s(this, A)[s(this, O)].forEach((t) => {
        t.classList.add(`${s(this, Y)}active`);
      });
  }),
  (ps = new WeakSet()),
  (dr = function () {
    s(this, M).children[0].classList.remove(`${s(this, pt).cssPrefix}dragging`),
      s(this, A).forEach((t) => {
        t.forEach((e) => {
          e.classList.remove(`${s(this, Y)}active`);
        });
      });
  }),
  (gs = new WeakSet()),
  (ur = function () {
    s(this, A).forEach((t) => {
      t.forEach((e) => {
        e.classList.remove(`${s(this, Y)}dragging`);
      });
    });
  }),
  (ce = new WeakSet()),
  (di = function (t, e) {
    for (let i = 0; i < t.length - 1; i++) if (t[i] < e && t[i + 1] >= e) return i;
    return t.length - 1;
  }),
  (ms = new WeakSet()),
  (fr = function () {
    if (s(this, O) === s(this, m) && s(this, V) === -1) return;
    const t = o(this, Ot, Se).call(this),
      e = t === 'right' ? s(this, m) - 1 : s(this, m) + 1,
      i = o(this, de, ui).call(this, e);
    t === 'right'
      ? (s(this, S)[s(this, m)] = s(this, S)[s(this, m)] + i)
      : (s(this, S)[s(this, m) + 1] = s(this, S)[s(this, m) + 1] - i),
      o(this, At, Pe).call(this, s(this, k), e, s(this, m)),
      o(this, At, Pe).call(this, s(this, A), e, s(this, m)),
      o(this, At, Pe).call(this, s(this, N), e, s(this, m)),
      s(this, A).forEach((n, l) => {
        n.forEach((c) => {
          c.style.left = `${s(this, S)[l]}px`;
        });
      }),
      o(this, Ss, vr).call(this);
  }),
  (Ot = new WeakSet()),
  (Se = function () {
    return s(this, V) > s(this, m) ? 'left' : 'right';
  }),
  (de = new WeakSet()),
  (ui = function (t) {
    const e = s(this, k)[t].width;
    return s(this, k)[s(this, m)].width - e;
  }),
  (At = new WeakSet()),
  (Pe = function (t, e, i) {
    if (e === -1) return;
    const n = t[e];
    (t[e] = t[i]), (t[i] = n);
  }),
  (bs = new WeakSet()),
  (pr = function () {
    a(this, J, !1), o(this, gs, ur).call(this), o(this, ps, dr).call(this), o(this, vs, gr).call(this);
  }),
  (vs = new WeakSet()),
  (gr = function () {
    a(this, O, -1), a(this, m, -1), a(this, V, -1);
  }),
  (ws = new WeakSet()),
  (mr = function () {
    s(this, U).forEach((t) => {
      t.classList.remove('data-den-header-cell-draggable');
    });
  }),
  (xs = new WeakSet()),
  (br = function () {
    s(this, H).removeEventListener('mousedown', s(this, vt)),
      s(this, M).removeEventListener('mousemove', s(this, mt)),
      document.removeEventListener('mouseup', s(this, bt));
  }),
  (Ss = new WeakSet()),
  (vr = function () {
    this.PubSub.publish('info:dragging:columns-reorder:done', {
      columnsOrder: s(this, N),
      context: new b('info:dragging:columns-reorder:done'),
    });
  });
var T,
  Z,
  kt,
  B,
  tt,
  wt,
  Tt,
  D,
  xt,
  qt,
  St,
  Ht,
  Ps,
  wr,
  Cs,
  xr,
  Ds,
  Sr,
  Es,
  Pr,
  $s,
  Cr,
  ys,
  Dr,
  Fs,
  Er,
  Rs,
  $r,
  Ls,
  yr,
  Is,
  Fr,
  ue,
  fi;
class an {
  constructor(t, e, i) {
    r(this, Ps);
    r(this, Cs);
    r(this, Ds);
    r(this, Es);
    r(this, $s);
    r(this, ys);
    r(this, Fs);
    r(this, Rs);
    r(this, Ls);
    r(this, Is);
    r(this, ue);
    r(this, T, void 0);
    r(this, Z, void 0);
    r(this, kt, void 0);
    r(this, B, void 0);
    r(this, tt, void 0);
    r(this, wt, void 0);
    r(this, Tt, void 0);
    r(this, D, void 0);
    r(this, xt, void 0);
    r(this, qt, void 0);
    r(this, St, void 0);
    r(this, Ht, void 0);
    (this.PubSub = i),
      a(this, T, t),
      a(this, Z, e),
      a(this, kt, !1),
      a(this, B, !1),
      a(this, tt, []),
      a(this, wt, []),
      a(this, St, []),
      a(this, Tt, []),
      a(this, D, null),
      a(this, xt, null),
      a(this, qt, []),
      s(this, Ht),
      o(this, Ps, wr).call(this),
      o(this, Cs, xr).call(this);
  }
  init() {
    a(this, tt, Array.from(s(this, T).querySelectorAll('[ref="headerCell"]'))),
      a(
        this,
        wt,
        Array.from(s(this, T).querySelector('[ref="headerMainCellsWrapper"]').querySelectorAll('[ref="headerCell"]'))
      ),
      a(this, St, ti(s(this, Z).columns)),
      o(this, Ds, Sr).call(this),
      o(this, Es, Pr).call(this),
      a(this, kt, !0);
  }
}
(T = new WeakMap()),
  (Z = new WeakMap()),
  (kt = new WeakMap()),
  (B = new WeakMap()),
  (tt = new WeakMap()),
  (wt = new WeakMap()),
  (Tt = new WeakMap()),
  (D = new WeakMap()),
  (xt = new WeakMap()),
  (qt = new WeakMap()),
  (St = new WeakMap()),
  (Ht = new WeakMap()),
  (Ps = new WeakSet()),
  (wr = function () {
    this.PubSub.subscribe('info:fetch:done', () => {
      s(this, kt) || this.init(),
        setTimeout(() => {
          a(this, Tt, Array.from(s(this, T).querySelectorAll('[ref="row"]')));
        }, 0);
    });
  }),
  (Cs = new WeakSet()),
  (xr = function () {
    this.PubSub.subscribe('command:rerendering:done', () => {
      a(this, tt, Array.from(s(this, T).querySelectorAll('[ref="headerCell"]'))),
        a(
          this,
          wt,
          Array.from(s(this, T).querySelector('[ref="headerMainCellsWrapper"]').querySelectorAll('[ref="headerCell"]'))
        ),
        a(this, St, ti(s(this, Z).columns));
    });
  }),
  (Ds = new WeakSet()),
  (Sr = function () {
    this.PubSub.subscribe('info:resizing:mousedown', o(this, $s, Cr).bind(this)),
      document.addEventListener('mousemove', o(this, Fs, Er).bind(this)),
      document.addEventListener('mouseup', o(this, ys, Dr).bind(this));
  }),
  (Es = new WeakSet()),
  (Pr = function () {
    this.PubSub.subscribe('info:dragging:columns-reorder:done', (t) => {
      a(this, St, t.data.columnsOrder);
    });
  }),
  ($s = new WeakSet()),
  (Cr = function (t) {
    s(this, T).children[0].classList.add(`${s(this, Z).cssPrefix}resizing`),
      a(this, D, t.data.target.parentElement),
      a(this, xt, o(this, ue, fi).call(this, s(this, D))),
      a(this, Ht, t.data.isPinnedRight),
      !(!s(this, D) || !s(this, D).parentElement) && (a(this, B, !0), a(this, qt, o(this, Is, Fr).call(this)));
  }),
  (ys = new WeakSet()),
  (Dr = function () {
    s(this, T).children[0].classList.remove(`${s(this, Z).cssPrefix}resizing`),
      s(this, B) &&
        (a(this, B, !1),
        this.PubSub.publish('info:resizing:done', {
          context: new b('info:resizing:done'),
        }));
  }),
  (Fs = new WeakSet()),
  (Er = function (t) {
    if (!s(this, B) || !s(this, D) || s(this, xt).some((i) => i === void 0)) {
      a(this, B, !1);
      return;
    }
    const e = s(this, Ht) ? -t.movementX : t.movementX;
    o(this, Rs, $r).call(this, e),
      o(this, Ls, yr).call(this, e),
      this.PubSub.publish('info:resizing:start', {
        currentColIndex: s(this, tt).indexOf(s(this, D)),
        newCurrentColWidth: parseInt(s(this, D).style.width, 10),
        context: new b('info:resizing:start'),
      });
  }),
  (Rs = new WeakSet()),
  ($r = function (t) {
    var n;
    const e = (n = s(this, D)) == null ? void 0 : n.style.width,
      i = parseInt(e || '0') + t;
    s(this, xt).forEach((l) => (l.style.width = `${i}px`));
  }),
  (Ls = new WeakSet()),
  (yr = function (t) {
    s(this, D) &&
      s(this, qt).forEach((e) => {
        const i = e.style.left;
        if (i === 'auto') return;
        const n = parseInt(i || '0') + t;
        o(this, ue, fi)
          .call(this, e)
          .forEach((c) => (c.style.left = `${n}px`));
      });
  }),
  (Is = new WeakSet()),
  (Fr = function () {
    var e;
    const t = parseInt(((e = s(this, D)) == null ? void 0 : e.style.left) || '0', 10);
    return s(this, wt).filter((i) => parseInt(i.style.left || '0', 10) > t);
  }),
  (ue = new WeakSet()),
  (fi = function (t) {
    if (!t || !t.parentElement) return [];
    const e = s(this, tt).indexOf(t),
      i = s(this, Tt).map((n) => n.querySelectorAll('[ref="cell"]')[e]);
    return [t, ...i];
  });
var Ms, Rr, Os, Lr, As, Ir;
class on {
  constructor(t, e) {
    r(this, Ms);
    r(this, Os);
    r(this, As);
    (this.PubSub = e),
      (this.activeHeaderFilters = {}),
      (this.options = t),
      (this.activeQuickFilter = { searchTerm: '', filterFn: t.quickFilterOptions.filterFn }),
      this.PubSub.subscribe('info:filtering:header-filter-changed', o(this, Ms, Rr).bind(this)),
      this.PubSub.subscribe('info:filtering:quick-filter-changed', o(this, As, Ir).bind(this));
  }
}
(Ms = new WeakSet()),
  (Rr = function (t) {
    const { field: e, type: i, state: n, isActive: l, filterFn: c } = t.data,
      d = { type: i, state: n, filterFn: c };
    o(this, Os, Lr).call(this, e, d, l);
    const f = {
      context: new b('info:filtering:active-filters-changed'),
      filters: this.activeHeaderFilters,
    };
    this.PubSub.publish('info:filtering:active-filters-changed', f);
  }),
  (Os = new WeakSet()),
  (Lr = function (t, e, i) {
    i ? (this.activeHeaderFilters[t] = e) : delete this.activeHeaderFilters[t];
  }),
  (As = new WeakSet()),
  (Ir = function (t) {
    const { searchTerm: e } = t.data;
    this.activeQuickFilter.searchTerm = e;
    const i = {
      context: new b('info:filtering:active-quick-filter-changed'),
      filter: this.activeQuickFilter,
    };
    this.PubSub.publish('info:filtering:active-quick-filter-changed', i);
  });
class hn {
  constructor(t) {
    (this.data = {}), (this.data = { ...this.data, ...t });
  }
  getValue(t) {
    return this.data[t];
  }
}
var j, zt, fe;
class ln {
  constructor(t, e) {
    r(this, j, void 0);
    r(this, zt, void 0);
    r(this, fe, void 0);
    (this.PubSub = e),
      a(this, j, ''),
      a(this, zt, 'asc'),
      a(this, fe, t),
      (this.activeSortersMap = /* @__PURE__ */ new Map()),
      this.PubSub.subscribe('command:sorting:start', (i) => {
        const n = new hn({ field: s(this, j), order: s(this, zt) }),
          { field: l, isMultiSort: c } = i.data,
          d = this.activeSortersMap.has(l),
          f = d ? this.activeSortersMap.get(l).order : '';
        let u;
        if (d)
          switch (f) {
            case 'asc':
              u = 'desc';
              break;
            case 'desc':
              u = '';
              break;
            default:
              u = 'asc';
              break;
          }
        else u = 'asc';
        i.data.order && (u = i.data.order), a(this, j, i.data.field);
        const x = s(this, fe).columns.find((Q) => Q.field === s(this, j)).sortOptions.comparator;
        if (
          Wt.triggerEvent('sortingStart', {
            field: l,
            order: u,
            comparator: x,
            sortFn: this.sort,
          }).defaultPrevented
        ) {
          a(this, j, n.getValue('field')), a(this, zt, n.getValue('order'));
          return;
        }
        c
          ? this.updateActiveSortersMap(l, u, x)
          : (this.activeSortersMap.clear(), this.updateActiveSortersMap(l, u, x)),
          this.PubSub.publish('command:fetch:sort-start', {
            caller: this,
            context: i.context,
            field: l,
            order: u,
            comparator: x,
            sortFn: this.sort,
            activeSorters: this.getActiveSortersArray(),
            isMultiSort: c,
          });
      });
  }
  updateActiveSortersMap(t, e, i) {
    const n = this.activeSortersMap.has(t);
    if (e === '' && n) {
      this.activeSortersMap.delete(t), this.updateSortIndexes();
      return;
    }
    if (n && e !== '') {
      const l = this.activeSortersMap.get(t);
      l.order = e;
    } else if (!n && e !== '') {
      const l = this.activeSortersMap.size;
      this.activeSortersMap.set(s(this, j), { field: t, order: e, comparator: i, sortIndex: l });
    }
  }
  updateSortIndexes() {
    let t = 0;
    for (const e of this.activeSortersMap.values()) e.sortIndex = t++;
  }
  getActiveSortersArray() {
    return Array.from(this.activeSortersMap.values()).sort((t, e) => t.sortIndex - e.sortIndex);
  }
  sort(t, e) {
    return (
      t.sort((i, n) => {
        let l = 0;
        for (const c of e) {
          const { order: d, field: f, comparator: u } = c;
          if (l === 0) {
            const g = d === 'asc',
              x = i[f],
              be = n[f],
              Q = u(x, be);
            l = g ? Q : Q * -1;
          } else break;
        }
        return l;
      }),
      t
    );
  }
}
(j = new WeakMap()), (zt = new WeakMap()), (fe = new WeakMap());
class Mr {}
var pe;
class cn extends Mr {
  constructor(e) {
    super();
    r(this, pe, void 0);
    a(this, pe, e);
  }
  getData(e) {
    return this.filterData(Gt(s(this, pe)), e.filtersOptions)
      .then((i) => this.quickFilterData(i, e.quickFilterOptions))
      .then((i) => this.sortData(i, e.sortingOptions))
      .then((i) => this.paginateData(i, e.paginationOptions));
  }
  filterData(e, i) {
    if (!i) return Promise.resolve(e);
    const l = Object.keys(i.filters).reduce(
      (c, d) =>
        c.filter((f) => {
          const u = i.filters[d].filterFn,
            g = i.filters[d].state,
            x = f[d];
          return u(g, x);
        }),
      e
    );
    return Promise.resolve(l);
  }
  paginateData(e, i) {
    if (!i) return Promise.resolve(e);
    const { firstRowIndex: n, lastRowIndex: l } = i,
      c = e.slice(n, l);
    return Promise.resolve(c);
  }
  sortData(e, i) {
    if (!i) return Promise.resolve(e);
    const { sortFn: n, activeSorters: l } = i,
      c = n(e, l);
    return Promise.resolve(c);
  }
  quickFilterData(e, i) {
    if (!i) return Promise.resolve(e);
    const { searchTerm: n, filterFn: l } = i,
      c = e.filter((d) => l(n, Object.values(d)));
    return Promise.resolve(c);
  }
}
pe = new WeakMap();
class dn extends Mr {
  getData(t) {
    return fetch('url' + new URLSearchParams(t)).then((e) => e.json());
  }
  sortData(t, e) {
    return fetch('url' + new URLSearchParams(e)).then((i) => i.json());
  }
  filterData(t, e) {
    return fetch('url' + new URLSearchParams(e)).then((i) => i.json());
  }
  quickFilterData(t, e) {
    return fetch('url' + new URLSearchParams(e)).then((i) => i.json());
  }
  paginateData(t, e) {
    return fetch('url' + new URLSearchParams(e)).then((i) => i.json());
  }
}
var ge, E, et, Ct, st, Dt, ks, Or, Ts, Ar, qs, kr, Hs, Tr, zs, qr;
class un {
  constructor(t, e) {
    r(this, et);
    r(this, st);
    r(this, ks);
    r(this, Ts);
    r(this, qs);
    r(this, Hs);
    r(this, zs);
    r(this, ge, void 0);
    r(this, E, void 0);
    (this.PubSub = e),
      a(this, ge, t),
      a(this, E, {}),
      o(this, ks, Or).call(this),
      o(this, Ts, Ar).call(this),
      o(this, qs, kr).call(this),
      o(this, Hs, Tr).call(this),
      o(this, zs, qr).call(this);
  }
}
(ge = new WeakMap()),
  (E = new WeakMap()),
  (et = new WeakSet()),
  (Ct = function (t) {
    return s(this, ge).getData(t);
  }),
  (st = new WeakSet()),
  (Dt = function (t, e) {
    const i = {
      caller: this,
      context: t,
      rows: e,
    };
    this.PubSub.publish('info:fetch:done', i);
  }),
  (ks = new WeakSet()),
  (Or = function () {
    this.PubSub.subscribe('command:fetch:start', (t) => {
      o(this, et, Ct)
        .call(this, s(this, E))
        .then((e) => {
          o(this, st, Dt).call(this, t.context, e);
        });
    });
  }),
  (Ts = new WeakSet()),
  (Ar = function () {
    this.PubSub.subscribe('command:fetch:sort-start', (t) => {
      (s(this, E).sortingOptions = {
        field: t.data.field,
        order: t.data.order,
        comparator: t.data.comparator,
        sortFn: t.data.sortFn,
        activeSorters: t.data.activeSorters,
      }),
        o(this, et, Ct)
          .call(this, s(this, E))
          .then((e) => {
            o(this, st, Dt).call(this, t.context, e), Wt.triggerEvent('sortingDone', s(this, E).sortingOptions);
          });
    });
  }),
  (qs = new WeakSet()),
  (kr = function () {
    this.PubSub.subscribe('info:filtering:active-quick-filter-changed', (t) => {
      (s(this, E).quickFilterOptions = {
        searchTerm: t.data.filter.searchTerm,
        filterFn: t.data.filter.filterFn,
      }),
        o(this, et, Ct)
          .call(this, s(this, E))
          .then((e) => {
            o(this, st, Dt).call(this, t.context, e);
          });
    });
  }),
  (Hs = new WeakSet()),
  (Tr = function () {
    this.PubSub.subscribe('info:filtering:active-filters-changed', (t) => {
      (s(this, E).filtersOptions = {
        filters: t.data.filters,
      }),
        o(this, et, Ct)
          .call(this, s(this, E))
          .then((e) => {
            o(this, st, Dt).call(this, t.context, e);
          });
    });
  }),
  (zs = new WeakSet()),
  (qr = function () {
    this.PubSub.subscribe('info:pagination:info-change:done', (t) => {
      (s(this, E).paginationOptions = {
        firstRowIndex: t.data.firstRowIndex,
        lastRowIndex: t.data.lastRowIndex,
      }),
        o(this, et, Ct)
          .call(this, s(this, E))
          .then((e) => {
            o(this, st, Dt).call(this, t.context, e);
          });
    });
  });
class fn {
  constructor(t, e, i) {
    (this.name = t), (this.data = e), (this.context = i), (this.key = '' + Math.random());
  }
}
var z, Ws, Hr;
class pn {
  constructor() {
    r(this, Ws);
    r(this, z, {
      'command:pagination:load-first-page:start': [],
      'command:pagination:load-prev-page:start': [],
      'command:pagination:load-next-page:start': [],
      'command:pagination:load-last-page:start': [],
      'info:pagination:data-change:done': [],
      'info:pagination:info-change:done': [],
      'info:pagination:page-size-change:done': [],
      'info:dragging:columns-reorder:done': [],
      'command:sorting:start': [],
      'info:filtering:get-active-quick-filter:done': [],
      'info:filtering:header-filter-changed': [],
      'info:filtering:active-filters-changed': [],
      'info:filtering:quick-filter-changed': [],
      'info:filtering:active-quick-filter-changed': [],
      'info:resizing:mousedown': [],
      'info:resizing:start': [],
      'info:resizing:done': [],
      'command:fetch:start': [],
      'command:fetch:sort-start': [],
      'info:fetch:done': [],
      'command:pin-column:start': [],
      'command:rerendering:done': [],
    });
  }
  publish(t, e) {
    if (!s(this, z)[t]) return;
    const i = new fn(t, e, e.context);
    s(this, z)[t].forEach((n) => {
      n(i);
    });
  }
  subscribe(t, e) {
    if (!s(this, z)[t]) throw new Error(`Could not subscribe: Unsupported event: ${t}`);
    return (
      s(this, z)[t].push(e),
      () => {
        o(this, Ws, Hr).call(this, t, e);
      }
    );
  }
}
(z = new WeakMap()),
  (Ws = new WeakSet()),
  (Hr = function (t, e) {
    s(this, z)[t] && (s(this, z)[t] = s(this, z)[t].filter((i) => i !== e));
  });
const gn = {
    pageSize: 10,
    pageSizeOptions: [5, 10, 25],
    ofText: 'of',
  },
  mn = (h, t) => ((h = h.toString().toLowerCase()), (t = t.toString().toLowerCase()), t.includes(h)),
  bn = {
    filterFn: mn,
  },
  vn = (h) => {
    const t = h.split('/').map((l) => Number(l)),
      [e, i, n] = t;
    return new Date(n, i - 1, e);
  },
  wn = (h, t) => (
    typeof h == 'string' && (h = h.toLowerCase()),
    typeof t == 'string' && (t = t.toLowerCase()),
    h === t ? 0 : h > t ? 1 : -1
  ),
  xn = {
    comparator: wn,
  },
  Sn = {
    cssPrefix: 'data-den-',
    mode: 'client',
    columns: [],
    defaultColDef: {
      sort: !1,
      sortOptions: xn,
      filter: !1,
      filterRenderer: Jr,
      filterOptions: {
        method: 'includes',
        debounceTime: 500,
        caseSensitive: !1,
        dateParserFn: vn,
        listOptions: [],
      },
      resize: !1,
      width: 200,
      cellRenderer: Nr,
    },
    rows: [],
    draggable: !1,
    pagination: !1,
    paginationOptions: gn,
    quickFilterOptions: bn,
    resizable: !1,
    rowHeight: 42,
    headerHeight: 42,
    multiSortKey: 'shift',
  },
  ve = (h) => h && typeof h == 'object',
  pi = (h, t) =>
    !ve(h) || !ve(t)
      ? t
      : (Object.keys(t).forEach((e) => {
          const i = h[e],
            n = t[e];
          Array.isArray(i) && Array.isArray(n)
            ? (h[e] = i.concat(n))
            : ve(i) && ve(n)
            ? (h[e] = pi(Object.assign({}, i), n))
            : (h[e] = n);
        }),
        h);
var Gs, Us, Vs, Ns, Bs, js, Pt, me, Qs, zr, Xs, Wr;
class $n {
  constructor(t, e) {
    r(this, Qs);
    r(this, Xs);
    r(this, Gs, void 0);
    r(this, Us, void 0);
    r(this, Vs, void 0);
    r(this, Ns, void 0);
    r(this, Bs, void 0);
    r(this, js, void 0);
    r(this, Pt, void 0);
    r(this, me, void 0);
    a(this, Pt, null), a(this, me, null), (this.PubSub = new pn());
    const i = o(this, Qs, zr).call(this, Sn, e);
    a(this, Pt, o(this, Xs, Wr).call(this, i)),
      s(this, Pt) && a(this, me, new un(s(this, Pt), this.PubSub)),
      a(this, Gs, new sn(t, i, this.PubSub)),
      a(this, Us, new ln(i, this.PubSub)),
      a(this, Vs, new on(i, this.PubSub)),
      a(this, Ns, new rn(i, this.PubSub)),
      a(this, Bs, e.draggable ? new nn(t, i, this.PubSub) : null),
      a(this, js, e.columns.some((n) => n.resize) ? new an(t, i, this.PubSub) : null);
  }
  on(t, e) {
    Wt.on(t, e);
  }
  sort(t, e) {
    const i = 'command:sorting:start';
    this.PubSub.publish(i, {
      context: new b(i),
      field: t,
      order: e,
    });
  }
  quickFilter(t) {
    const i = {
      context: new b('info:filtering:quick-filter-changed'),
      searchTerm: t,
    };
    this.PubSub.publish('info:filtering:quick-filter-changed', i);
  }
  pinColumn(t, e) {
    const i = 'command:pin-column:start';
    this.PubSub.publish(i, {
      context: new b(i),
      pin: t,
      colIndex: e,
    });
  }
}
(Gs = new WeakMap()),
  (Us = new WeakMap()),
  (Vs = new WeakMap()),
  (Ns = new WeakMap()),
  (Bs = new WeakMap()),
  (js = new WeakMap()),
  (Pt = new WeakMap()),
  (me = new WeakMap()),
  (Qs = new WeakSet()),
  (zr = function (t, e) {
    const i = pi(Gt(t), e);
    return (
      i.columns.forEach((n, l) => {
        i.columns[l] = pi(Gt(i.defaultColDef), n);
      }),
      i
    );
  }),
  (Xs = new WeakSet()),
  (Wr = function (t) {
    const { mode: e, rows: i } = t;
    return e === 'client' && i.length ? new cn(t.rows) : e === 'server' ? new dn() : null;
  });
export {
  $n as DataDen,
  cn as DataDenClientDataLoaderStrategy,
  Mr as DataDenDataLoaderStrategy,
  Nr as DataDenDefaultCellRenderer,
  Dn as DataDenHeaderDateFilterRenderer,
  Cn as DataDenHeaderNumberFilterRenderer,
  En as DataDenHeaderSelectFilterRenderer,
  Jr as DataDenHeaderTextFilterRenderer,
  dn as DataDenServerDataLoaderStrategy,
};
//# sourceMappingURL=data-den.es.min.js.map
