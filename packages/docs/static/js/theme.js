class ThemeSwitcher {
  ...
  setActiveDropdownItem(theme) {
    ...
    this.element.querySelector(`[data-theme-icon=${theme}]`).classList.add('text-primary-500');
    this.element.querySelector(`[data-theme-name=${theme}]`).classList.add('text-primary-500');
  }
  ...
  onThemeSwitcherShortCut() {
    if (!('theme' in localStorage)) {
      document.querySelector('html').classList.contains('dark') ? this.setLightTheme() : this.setDarkTheme();
    } else if (localStorage.theme === 'dark') {
      this.setLightTheme();
    } else {
      this.setDarkTheme();
  ...
  initCustomTogglers() {
    ...
    this.customTogglers.forEach((customToggler) => {
      customToggler.addEventListener('change', () => this.toggleDarkOrLightTheme());
    });
  }
